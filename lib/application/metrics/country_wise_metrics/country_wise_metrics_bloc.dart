...
part 'country_wise_metrics_bloc.g.dart';

@injectable
class CountryWiseMetricsBloc
    extends HydratedBloc<CountryWiseMetricsEvent, CountryWiseMetricsState> {
  final IMetricsRepository _repository;
  final DateService _dateService;
  final IAccountRepository _accountRepository;

  CountryWiseMetricsBloc(
    this._repository,
    this._dateService,
    this._accountRepository,
  ) : super(CountryWiseMetricsState.initial()) {
      ...
      requstedLifeTime: (e) async {
        ...
        await acOpenDateEither.fold(
          ...
          (s) async {
            ...
            final result = await _repository.getCountryMetrics(range);
            result.fold(
              (l) => emit(
                state.copyWith(
                  isLoading: false,
  ...
  //           await _accountRepository.getAccoutOpeningDate();
  //       await acOpenDateEither.fold(
  //         (f) async => emit(const CountryWiseMetricsState.failed(
  ...
  @override
  CountryWiseMetricsState? fromJson(Map<String, dynamic> json) {
    cprint('PRSL', 'FromJSon');
    try {
      cprint('PRSL Err', '');
      return CountryWiseMetricsState.fromJson(json);
    } catch (e) {
      return null;
    }
  }
  ...
  @override
  Map<String, dynamic>? toJson(CountryWiseMetricsState state) {
    return state.toJson();
  }
}
